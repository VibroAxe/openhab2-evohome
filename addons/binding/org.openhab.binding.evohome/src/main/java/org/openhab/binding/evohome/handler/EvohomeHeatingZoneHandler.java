...
import org.openhab.binding.evohome.internal.configuration.EvohomeOverrideModeConstants;
            ...
            if (bridge != null) {
                ...
                if (EvohomeBindingConstants.ZONE_SET_POINT_CHANNEL.equals(channelId)
                    ...
                    if (newTemp >= 5 && newTemp <= 35) {
                        ...
                        switch (overrideMode) {
                            case EvohomeOverrideModeConstants.TEMPORARY_OVERRIDE_SCHEDULE_NEXT:
                                bridge.setTemporarySetPoint(getEvohomeThingConfig().id, newTemp,
                                        getNextSetPointChange(schedule));
                                break;
                            case EvohomeOverrideModeConstants.TEMPORARY_OVERRIDE_TIME_ADD:
                                bridge.setTemporarySetPoint(getEvohomeThingConfig().id, newTemp,
